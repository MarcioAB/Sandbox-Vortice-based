...
public static class D3D12_Base
{
    ...
    public static ID3D12Fence AuxFence;
    ...
    public static void D3D_Dispose()
    {
        WaitForGpu();
#if DEBUG
        DebugLayer.DebugLayer_DXGIbased("D3D_Dispose", true); // Display Warning messages. Stop on Error.
#endif
        GPU.Dispose();
        AuxFence.Dispose();
        DSV12.Dispose();
    ...
    public static ID3D12Resource CreateBufferResourceAndUploadData<T>(T[] buffer, int nameID)
    {
        ...
        // Removed porque sempre passa de 1ª
        // WaitForGpu(">>>>> update vertex buffer"); // garante que a GPU esta pronta para liberar o recurso "source". Sem isso temos um Exception no Dispose() 
    ...
    public static ID3D12Resource CreateTextureResourceAndUploadData(ResourceDescription textureDesc, ReadOnlySpan<byte> textureData)
    {
        ...
        WaitForGpu(); // garante que a GPU esta livre antes do disposal desse resource.
        srcResource.Dispose();
    ...
    static void CreateFence()
    {
        ...
        // 2 fences:
        //    1 for MoveToNextObject() based on frame counting and
        //    1 for WaitToGpu() based on {set 0 from CPU side, set N from GPU side, wait for N from CPU side}
        GPU = DEV12_8.CreateFence();
        GPU.Name = "Frame Fence";
        AuxFence = DEV12_8.CreateFence(); // Melhorar um pouco aqui REUTILIZANDO uma unica fence ...
        AuxFence.Name = "AUX Fence";
    ...
    // The strategy for this fence is set 0 from CPU side, set 1234 from GPU side and wait for the 1234 from CPU side.
    // Obs1: If GPU is busy it will not process the 1234 signal that was set on the queue until "GPU is free".
    // Obs2: "GPU is free" because the 1234 signal is the last command set on the queue. 
    public static void WaitForGpu()
    {
        AuxFence.Signal(0);
        Q12.Signal(AuxFence, 1234); // Q12 só vai conseguir colocar esse 0 na Fence quando terminar essa tarefa
        AuxFence.SetEventOnCompletion(1234, CPUsynchronizer);
        CPUsynchronizer.WaitOne();
    }

    static void MoveToNextFrame()
    {
        ulong thisFrame = CPUcompleted[_backBufferIndex]; // Schedule a Signal command in the queue.
        _backBufferIndex = SWAP12_4.CurrentBackBufferIndex; // Update the back buffer index.
        ulong frameReadyToGo = CPUcompleted[_backBufferIndex];
        Q12.Signal(GPU, thisFrame); // add this fence signal in the CommandQueue. When the CommandQueue executes this, it updates the fence value.

        // A principio a GPU é mais rapida que a CPU então uma fila com 2 backbuffers deveria dar.
        // Enquanto a GPU executa o frame que foi colocado pela CPU no backbuffer 1,
        // a CPU já esta trabalhando no proximo frame no backbuffer 2.
        // Porem eventualmente a CPU pode terminar o trabalho no backbuffer 2 antes da GPU liberar o backbuffer 1.
        // Nesse caso alguem tem que segurar a CPU ate que a GPU informe que liberou o backbuffer 1.
        //
        // É isso que esse logica aqui faz:
        // Se a GPU indicar que o frame que ela acabou de executar é MENOR que o proximo frame que esta pronto na fila,
        // o "WaitOne" faz a CPU parar de colocar frames na fila ate a GPU indicar que ja executou esse frame que ja estava pronto na fila.
        //
        // O tamanho dessa fila é o numero de backbuffers.
        // Se a fila tiver 1000 backbuffers, dificilmente a CPU vai ter que parar.
        // Esse AuxFunctions.waitCount serve para ver (em tempo real) quantas vezes a CPU teve que parar.
        // Se a CPU esta parando muitas vezes, aumentar o numero de backbuffers.
        //
        if (GPU.CompletedValue < frameReadyToGo)
        {
            AuxFunctions.waitCount++;
            GPU.SetEventOnCompletion(frameReadyToGo, CPUsynchronizer).CheckError();
            CPUsynchronizer.WaitOne(); // hold CPU until GPU did not signal it is there.
        }
        // Everything OK ... move on.
        CPUcompleted[_backBufferIndex] = thisFrame + 1;
    }
}
