...
// SandBox MVP introduction: L1
// constant buffer: 64-bytes constant buffer for MVP ( Matrix4x4 )
// (A) 1 vertex buffer for vertex coords <T1>
// (B) 2 vertex buffers: one for vertex coords <T1>, other for anything else <T2>
/// <summary>MVP, 1 buffer</summary>
public class DrawSetBase_NoMulti_NoTex_NoIndex : D3D11_ShaderCompiler
{
    public int qtd_vertices; // precisa ser public porque é usado para calcular o numero total de vertices no Models.
    public Matrix4x4 model; // precisa ser public porque esse input é feito no Models.
    public PrimitiveTopology tipo;
    public ID3D11Buffer vertexBuffer;
    public int rasterizerNDX;
    ...
    public DrawSetBase_NoMulti_NoTex_NoIndex(string sh) : base(sh)
    {
        model = Matrix4x4.Identity;
        rasterizerNDX = (int)RenderMode.Solid; // default. Use SetRenderMode to change
        v1 = new D3D11_MVP();
    ...
    public void SetVertexData<T>((PrimitiveTopology tipo, T[] vertex) data) where T : unmanaged // D3D11 CreateBuffer precisa que o <T> seja unmanaged
    {
        qtd_vertices = data.vertex.Length;
        tipo = data.tipo;
    ...
    public void Draw() // executed at EACH frame, for EACH DrawSet
    {
        ...
        DC11.IASetInputLayout(inputLayout: inputLayout);
        DC11.IASetPrimitiveTopology(topology: tipo);
        DC11.IASetVertexBuffer(slot: MyShaderSources.vertexSlot, buffer: vertexBuffer, stride: stride); // slot used here MUST BE the same as defined in "InputElementDescription" for this element.
...
/// <summary>MVP, 2 buffers</summary>
public class DrawSetBase_NoMulti_NoTex_NoIndex2 : D3D11_ShaderCompiler
{
    public int qtd_vertices; // precisa ser public porque é usado para calcular o numero total de vertices no Models.
    public Matrix4x4 model; // precisa ser public porque esse input é feito no Models.
    public PrimitiveTopology tipo;
    ...
    public DrawSetBase_NoMulti_NoTex_NoIndex2(string sh) : base(sh)
    {
        model = Matrix4x4.Identity;
        rasterizerNDX = (int)RenderMode.Solid; // default. Use SetRenderMode to change
        v1 = new D3D11_MVP();
    ...
    public void SetVertexData<T1, T2>((PrimitiveTopology tipo, T1[] vertex1) data, T2[] vertex2) where T1 : unmanaged where T2 : unmanaged
    {
        if (data.vertex1.Length != vertex2.Length) throw new Exception();
        qtd_vertices = data.vertex1.Length;
        tipo = data.tipo;

        vertexBuffers = new ID3D11Buffer[] {
            DEV11.CreateBuffer(data.vertex1, BindFlags.VertexBuffer),
            DEV11.CreateBuffer(vertex2, BindFlags.VertexBuffer)};
        strides = new int[] { Unsafe.SizeOf<T1>(), Unsafe.SizeOf<T2>() };
        offsets = new int[] { 0, 0 };
    ...
    public void Draw() // executed at EACH frame, for EACH DrawSet
    {
        ...
        DC11.IASetInputLayout(inputLayout: inputLayout);
        DC11.IASetPrimitiveTopology(topology: tipo);
        DC11.IASetVertexBuffers(firstSlot: MyShaderSources.vertexSlot, vertexBuffers: vertexBuffers, strides: strides, offsets: offsets);
